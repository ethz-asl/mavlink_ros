...
#define MAVLINK_MSG_ID_ASLCTRL_DATA_LEN 93
#define MAVLINK_MSG_ID_203_LEN 93

#define MAVLINK_MSG_ID_ASLCTRL_DATA_CRC 60
#define MAVLINK_MSG_ID_203_CRC 60
...
#define MAVLINK_MESSAGE_INFO_ASLCTRL_DATA { \
	...
	{  { "timestamp", NULL, MAVLINK_TYPE_UINT64_T, 0, 0, offsetof(mavlink_aslctrl_data_t, timestamp) }, \
         ...
         { "YawAngle", NULL, MAVLINK_TYPE_FLOAT, 0, 52, offsetof(mavlink_aslctrl_data_t, YawAngle) }, \
         { "YawAngleRef", NULL, MAVLINK_TYPE_FLOAT, 0, 56, offsetof(mavlink_aslctrl_data_t, YawAngleRef) }, \
         { "RollAngle", NULL, MAVLINK_TYPE_FLOAT, 0, 60, offsetof(mavlink_aslctrl_data_t, RollAngle) }, \
         { "RollAngleRef", NULL, MAVLINK_TYPE_FLOAT, 0, 64, offsetof(mavlink_aslctrl_data_t, RollAngleRef) }, \
         { "p", NULL, MAVLINK_TYPE_FLOAT, 0, 68, offsetof(mavlink_aslctrl_data_t, p) }, \
         { "pRef", NULL, MAVLINK_TYPE_FLOAT, 0, 72, offsetof(mavlink_aslctrl_data_t, pRef) }, \
         { "r", NULL, MAVLINK_TYPE_FLOAT, 0, 76, offsetof(mavlink_aslctrl_data_t, r) }, \
         { "rRef", NULL, MAVLINK_TYPE_FLOAT, 0, 80, offsetof(mavlink_aslctrl_data_t, rRef) }, \
         { "uAil", NULL, MAVLINK_TYPE_FLOAT, 0, 84, offsetof(mavlink_aslctrl_data_t, uAil) }, \
         { "uRud", NULL, MAVLINK_TYPE_FLOAT, 0, 88, offsetof(mavlink_aslctrl_data_t, uRud) }, \
         { "aslctrl_mode", NULL, MAVLINK_TYPE_UINT8_T, 0, 92, offsetof(mavlink_aslctrl_data_t, aslctrl_mode) }, \
         } \
}
...
static inline uint16_t mavlink_msg_aslctrl_data_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint64_t timestamp, uint8_t aslctrl_mode, float h, float hRef, float hRef_t, float PitchAngle, float PitchAngleRef, float q, float qRef, float uElev, float uThrot, float uThrot2, float aZ, float YawAngle, float YawAngleRef, float RollAngle, float RollAngleRef, float p, float pRef, float r, float rRef, float uAil, float uRud)
{
	...
	_mav_put_float(buf, 52, YawAngle);
	_mav_put_float(buf, 56, YawAngleRef);
	_mav_put_float(buf, 60, RollAngle);
	_mav_put_float(buf, 64, RollAngleRef);
	_mav_put_float(buf, 68, p);
	_mav_put_float(buf, 72, pRef);
	_mav_put_float(buf, 76, r);
	_mav_put_float(buf, 80, rRef);
	_mav_put_float(buf, 84, uAil);
	_mav_put_float(buf, 88, uRud);
	_mav_put_uint8_t(buf, 92, aslctrl_mode);
